...
pip install -e 'git+https://github.com/adaptiveapplications/ckan.git#egg=ckan'
...
#than going back to current version 35.0.0
pip install setuptools==36.1


echo "# CONFIGURE THE CKAN 2.8"
echo "################################"
...
sed -i '60d' /etc/ckan/development.ini
sed -i '60i ckan.site_url = http://'`hostname -I | cut -d" " -f2` /etc/ckan/development.ini

sed -i '79d' /etc/ckan/development.ini
sed -i '79i ckan.site_id = ckan.local' /etc/ckan/development.ini
sed -i '80d' /etc/ckan/development.ini
sed -i '80i solr_url = http://127.0.0.1:8983/solr/ckan' /etc/ckan/development.ini


# SETUP LOCAL STORAGE FOR CKAN 
mkdir /var/lib/ckan
chown -R www-data:$VMUSER /var/lib/ckan
chmod 775 /var/lib/ckan

#uncomment the ckan.storage setting
sed -i '157s/.//' /etc/ckan/development.ini
...
sed -i '104s/$/ resource_proxy datastore datapusher/' /etc/ckan/development.ini

#uncomment the datapusher.url setting
sed -i '166s/.//' /etc/ckan/development.ini
...
sed -i '104s/$/ recline_grid_view recline_graph_view recline_map_view webpage_view/' /etc/ckan/development.ini
...
sed -i '104s/$/ spatial_metadata spatial_query/' /etc/ckan/development.ini
sed -i '183i # Spatial Extension Settings' /etc/ckan/development.ini
sed -i '184i ckan.spatial.srid = 4326' /etc/ckan/development.ini

# add geoview to the virtualenv
pip install ckanext-geoview
sed -i '104s/$/ geo_view geojson_view wmts_view/' /etc/ckan/development.ini
...
cp /vagrant/artifacts/datapusher.apache2-4.conf /etc/apache2/sites-available/datapusher.conf
cp /vagrant/artifacts/datapusher.wsgi /etc/ckan/ 
